...
const DisplayEnglishQuestions = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:8080/question/english-questions/type/${topic}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setEnglishQuestions(response.data);
      } catch (error) {
  ...
  const handleReplace = async () => {
    const token = sessionStorage.getItem('token');
    try {
      await axios.put(`http://localhost:8080/question/english-questions/${editingQuestion.id}`, editingQuestion, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setEnglishQuestions((prevQuestions) =>
  ...
  return (
    <Paper elevation={3} style={{ padding: '20px', maxWidth: '800px', margin: '20px auto', borderRadius: '10px' }}>
      ...
      <Typography variant="h4" gutterBottom align="center" style={{ color: '#333' }}>
        English Questions for Topic: {topic}
      </Typography>
      <TextField
        fullWidth
...
export default DisplayEnglishQuestions;